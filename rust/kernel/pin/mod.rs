...
//!         init_from_closure(
//!             |mut this: PinUninit<'_, Self>| -> InitResult<'_, Self, Infallible> {
...
/// #[pin_init]
/// struct ManyPin(#[pin] NeedPin, usize);
/// let p = Box::pin_with(init_pin!(ManyPin(NeedPin::new(), 0,)));
/// # }
/// ```
///
/// You could apply it to unit-structs (but probably not very useful):
...
macro_rules! init_stack {
    ($var:ident = $init:expr) => {
        let mut storage = $crate::pin::__private::StackWrapper::new();
        let $var = unsafe { ::core::pin::Pin::new_unchecked(&mut storage) }
            .init($crate::pin::init_pin!($init));
    };
}
