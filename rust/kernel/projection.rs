...
pub unsafe trait Field<Base> {
    ...
    unsafe fn reverse_map(ptr: *const Self::Type) -> *const Base {
        let container = core::mem::MaybeUninit::<Base>::uninit();
        let container_ptr = container.as_ptr();
        let offset = unsafe {
            Self::map(container_ptr)
                .cast::<u8>()
                .offset_from(container_ptr.cast::<u8>())
        };
        unsafe { ptr.cast::<u8>().offset(offset).cast() }
    }
}
...
macro_rules! field {
    ($name:ident) => {
        $crate::projection::FieldName<
            { $crate::projection::field_name_hash(core::stringify!($name)) },
        >
    };
}
        ...
        match $a {
            __expr => unsafe {
                $crate::projection::Projectable::<
                    _,
                    $crate::projection::FieldName<
                        { $crate::projection::field_name_hash(core::stringify!($b)) },
                    >,
