...
use quote::{format_ident, quote as quote2, quote_spanned as quote_spanned2, ToTokens};
use syn::{
    punctuated::Punctuated, Data, DeriveInput, Error, Fields, GenericParam, Generics, Member,
    Result,
};

pub(crate) fn pin_field(input: TokenStream) -> Result<TokenStream> {
    let DeriveInput {
        ident,
        generics,
        data,
    ...
    for i in 0..field_name.len() {
        ...
        let wrapper_ty = if has_pin[i] {
            quote_spanned2!(mixed_site => core::pin::Pin<&'__field_projection mut __FieldProjection>)
        } else {
            quote_spanned2!(mixed_site => &'__field_projection mut __FieldProjection)
        };

        let wrapper_maybe_uninit_ty = if has_pin[i] {
            quote_spanned2!(mixed_site => core::pin::Pin<&'__field_projection mut core::mem::MaybeUninit<__FieldProjection>>)
        } else {
            quote_spanned2!(mixed_site => &'__field_projection mut core::mem::MaybeUninit<__FieldProjection>)
        };

        builder.push(quote_spanned2! {mixed_site =>
            unsafe impl<
                #(#generics,)*
        ...
        if has_pin[i] {
            unpin_guard_builder.push(quote_spanned2! {mixed_site =>
                #field_idx: #ty,
            });
        } else {
            unpin_guard_builder.push(quote_spanned2! {mixed_site =>
                #field_idx: kernel::projection::AlwaysUnpin<#ty>,
            });
        }
    }
    ...
    let guard = quote_spanned2! {mixed_site => const _: () = {
