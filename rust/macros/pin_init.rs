...
pub(crate) fn pin_init_attr(_attr: TokenStream, input: TokenStream) -> Result<TokenStream> {
    let mut input: ItemStruct = syn::parse2(input)?;
    let attrs = std::mem::take(&mut input.attrs);
    Ok(quote! {
        #(#attrs)*
        #[derive(kernel::pin::__private::PinInit)]
        #input
    })
}

pub(crate) fn pin_init_derive(input: TokenStream) -> Result<TokenStream> {
    let DeriveInput {
        vis,
        ident,
        generics,
    ...
    let mut builder = Vec::new();
    {
        ...
        builder.push(quote_spanned! {mixed_site=>
            ...
            #vis struct #builder_ident<
                ...
                #typestate_impl
            > #where_clause {
                ptr: kernel::pin::PinUninit<#this_lifetime, #ident<#(#ty_generics),*>>,
            }
            ...
            > #where_clause {
                #[inline]
                fn __init_err<#error_ident>(mut self, err: #error_ident) -> kernel::pin::InitErr<#this_lifetime, #error_ident> {
        ...
        let fn_item = if has_pin[i] {
            quote_spanned! {mixed_site=>
                #field_vis fn #fn_name<#error_ident, #fn_ident>(mut self, f: #fn_ident) -> ::core::result::Result<#builder_ident<
                    ...
                    #(,#ty_generics)*
                    #typestate_ty_post
                >, kernel::pin::InitErr<#this_lifetime, #error_ident>>
                    where #fn_ident: kernel::pin::Init<#ty, #error_ident>
                {
                    let base = self.ptr.get_mut().as_mut_ptr();
                    ...
                    let pin = unsafe { kernel::pin::PinUninit::new(&mut *(ptr as *mut ::core::mem::MaybeUninit<_>)) };
                    match f.__init(pin) {
                        Ok(_) => (),
...

    {
        ...
        builder.push(quote_spanned! {mixed_site=>
            ...
            > #where_clause {
                #[inline]
                pub fn __init_ok(mut self) -> kernel::pin::InitOk<#this_lifetime, #ident<#(#ty_generics),*>>{
            ...
            impl<
                #this_lifetime
                #(,#generics)*
            > kernel::pin::Initable<#this_lifetime> for #ident<#(#ty_generics),*> #where_clause {
                #[doc(hidden)]
                ...
                fn __pin_init_builder(
                    this: kernel::pin::PinUninit<#this_lifetime, Self>,
        ...
        match expr {
            Expr::Path(path) if looks_like_tuple_struct_name(path) => {
                ...
                *expr = syn::parse2(quote_spanned! {Span::mixed_site()=>
                    kernel::pin::init_from_closure(move |this| {
                        use kernel::pin::Initable;
...
pub(crate) fn init_pin(input: TokenStream) -> Result<TokenStream> {
