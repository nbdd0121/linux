...
const NUM_LIBOS_ARGUMENT: usize = GSP_PAGE_SIZE / size_of::<LibosMemoryRegionInitArgument>();

/// GSP runtime data.
#[pin_data]
pub(crate) struct Gsp {
    /// Libos arguments.
    pub(crate) libos: Coherent<[LibosMemoryRegionInitArgument; NUM_LIBOS_ARGUMENT]>,
    /// Init log buffer.
    loginit: LogBuffer,
    ...
    pub(crate) fn new(pdev: &pci::Device<device::Bound>) -> impl PinInit<Self, Error> + '_ {
        pin_init::pin_init_scope(move || {
            ...
            Ok(try_pin_init!(Self {
                libos: Coherent::zeroed(dev, GFP_KERNEL)?,
                loginit: LogBuffer::new(dev)?,
                logintr: LogBuffer::new(dev)?,
                logrm: LogBuffer::new(dev)?,
                cmdq: Cmdq::new(dev)?,
                rmargs: Coherent::zeroed(dev, GFP_KERNEL)?,
                _: {
